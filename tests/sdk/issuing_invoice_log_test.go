...
func TestIssuingInvoiceLogQuery(t *testing.T) {
	...
	logs, errorChannel := IssuingInvoiceLog.Query(params, nil)
	loop:
	for {
		select {
...
func TestIssuingInvoiceLogGet(t *testing.T) {
	...
	logs, errorChannel := IssuingInvoiceLog.Query(paramsQuery, nil)
	loop:
	for {
		select {
		case err := <-errorChannel:
	...
	for _, log := range logList {
		getLog, err := IssuingInvoiceLog.Get(log.Id, nil)
		if err.Errors != nil {
			for _, e := range err.Errors {
				t.Errorf("code: %s, message: %s", e.Code, e.Message)
			}
